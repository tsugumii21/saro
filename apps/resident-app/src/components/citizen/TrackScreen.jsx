...
export default function TrackScreen() {
  ...
  const loadMine = useCallback(async () => {
    ...
    // Demo rows come from the exact source used by the public Map.
    if (isResident) {
  ...
  return (
    <div ref={topRef} className="flex flex-col gap-6 px-4 pb-8 pt-5">
      ...
      {error && (
        <div role="alert" className="border border-alert bg-alert-wash px-3 py-2.5 text-alert">
          <p className="t-body-sm font-bold">Report Not Found</p>
          <p className="t-body-sm mt-1">{error}</p>
        </div>
      )}
      ...
      {lightboxUrl && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-ink/80 p-4" role="dialog" aria-modal="true">
          ...
          <img src={lightboxUrl} alt="Report evidence enlarged" className="max-h-[88vh] max-w-full object-contain" />
        </div>
      )}
