-- Admin-only deletion path. Cascades audit/media rows, removes stored media,
-- and repairs any cluster the report belonged to.
...
begin
  ...
  select array_agg(cr.cluster_id)
    into affected_clusters
  from public.cluster_reports cr
  where cr.report_id = p_report_id;
...
comment on function public.delete_report(uuid) is
  'Admin-only permanent report removal with media and cluster cleanup.';
