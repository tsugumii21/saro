...
import {
  ...
  CloudOff, Info, UserPlus, X, MessageSquare, Image as ImageIcon, ShieldCheck, ArrowRight,
  Copy, Check, Activity,
} from "lucide-react";
import { StatusTag, TrackingCode, HazardMap } from "@saro/ui";
import {
  getReportByTrackingCode, getStatusHistory, getOffices, getReportMedia,
  getReportsByDevice, getMyReports, getOriginalMapDemoReports, saroEvents,
...
export default function TrackDesktop() {
  ...
  const loadMine = useCallback(async () => {
    ...
    if (isResident) {
      for (const row of getOriginalMapDemoReports()) {
        merged.set(row.tracking_code, { ...row, origin: "map" });
      }
    }
    ...
    <div className="flex h-full w-full overflow-hidden bg-canvas font-sans">
      ...
      <aside className="flex w-[400px] shrink-0 flex-col border-r border-line bg-surface overflow-y-auto">
        ...
        <div className="flex flex-col gap-4.5 p-5">
          ...
          {error && (
            <div role="alert" className="border border-alert bg-alert-wash px-3.5 py-2.5 text-alert rounded-xs">
              <p className="text-xs font-bold">Report Not Found</p>
                    ...
                    >
                      ...
                      <button
                        ...
                        onClick={() => {
                          ...
                          navigate(`/track?code=${encodeURIComponent(r.tracking_code)}`);
                          search(r.tracking_code);
                        }}
                        className="saro-btn saro-btn-primary saro-btn-sm saro-btn-block mt-0.5"
                      >
                        View Whole Report
                      </button>
                    </article>
      ...
      >
        {/* Idle Guide State — anchored to the top of the overflow-y-auto panel so the
            desk panel reads immediately instead of sitting below a band of empty space. */}
        {!report && !searching && (
            ...
            <div className="border border-line bg-surface p-6 rounded-md shadow-xs flex flex-col gap-4">
              ...
              <div className="pt-3 border-t border-line flex flex-col gap-2">
                ...
                <div className="flex flex-wrap gap-2">
                  {mine.filter((demo) => demo.origin === "map").slice(0, 4).map((demo) => (
                    <button
                    ...
                    >
                      ...
                      <span className="text-xs text-ink-muted group-hover:text-brand font-medium">
                        · {demo.category_label ?? demo.category}
                      </span>
          ...
          <div className="p-6 max-w-5xl mx-auto flex flex-col gap-6">
            ...
            >
              <div className="border-b border-rule p-5 flex items-center justify-between flex-wrap gap-4">
                <div className="flex items-center gap-3">
                  <div>
                    <TrackingCode code={report.tracking_code} size="xl" showCopy={false} />
